...
#define SPI_MAX_PKT_LEN						128
//#define SPI_MAX_DAT_LEN					 (SPI_MAX_PKT_LEN - SPI_FRM_LEN)
#define SPI_MAX_DAT_LEN						SPI_MAX_PKT_LEN
...
#define SPI_RDYIn									P0_5
#define SPI_RDYIn_BIT							BV(5)
...
#undef	PxDIR
#if ((HAL_UART_SPI == 2) && defined HAL_SPI_MASTER)
#define PxDIR											P1DIR
#define SPI_RDYOut								 P1_4
#define SPI_RDYOut_BIT						 BV(4)
#elif ((HAL_UART_SPI == 1) && defined HAL_SPI_MASTER)
#define PxDIR											P0DIR
...
// modified send procedure 7.15
//	spiTxPkt[SPI_LEN_IDX] = len;	//delete
...
#if 0	//modified by lan 15.8.3, control by app
	SPI_SET_CSn_OUT();
#endif
...
static void spiParseRx(void)
{
	while(!SPI_NEW_RX_BYTE(spiRxIdx-1))
	{
//		UxDBUF = 0;
//		while (UxCSR & CSR_ACTIVE);
	}
	
	uint8 ch = SPI_GET_RX_BYTE(spiRxIdx-1);
	SPI_CLR_RX_BYTE(spiRxIdx-1);
//	SPI_LEN_T_INCR(spiRxIdx);

	spiRxDat[spiRxTail] = ch;

//	SPI_LEN_T_INCR(spiRxTail);
}
/*
static void spiParseRx(void)
{
