 ...
 * GLOBAL VARIABLES
 */
// Default RF frequency params: work 433 set 433 upgrade 433
...
__root const uint8 RFM96SpreadFactorTbl[7] =
{
 ...
 uint8	gb_RxData[GMS_PKT_MAX_LEN];											 //Receive data buffer
#endif
...
{ 
		...
		//delayms(100);
		uint8 RFM96Data[GMS_PKT_MAX_LEN];
		uint8 RF96Datalen;
		
		osal_memset(RFM96Data,0,sizeof(RFM96Data));
		osal_memcpy(RFM96Data,buf,len);
		...
		RFM96Data[i]=sum;
		RF96Datalen = len+1;
		SPIWrite(LR_RegPayloadLength+ RF96Datalen);						 //RegPayloadLength 21byte
		BurstWrite(0x00, (uint8 *)RFM96Data, RF96Datalen);
