...
#define IDLE_PWR_HOLD_PERIOD		200
...
#define GM_TEMPR_MSB_REG				0x0E
#define GM_TEMPR_LSB_REG				0x0F

//GM base temperature
#define GM_BASE_TEMPR 					25

// 8 average 15Hzoutput rate
#define SET_GM_NORMAL					0x00//0x70
...
#define HAL_PIN_INT_ENABLE()		st( IEN2 |=	BV(4); )
#define HAL_PIN_INT_DISABLE()		st( IEN2 &=	~(BV(4)); )
...
#define HAL_PIN_INT_ENABLE()		st( IEN1 |=	BV(5); )
#define HAL_PIN_INT_DISABLE()		st( IEN1 &=	~(BV(5)); )
...
#define GM_DRDY_INT_INT_ENABLE()		st( GM_DRDY_INT_PXIEN |= GM_DRDY_INT_IE; )
#define GM_DRDY_INT_INT_DISABLE()		st( GM_DRDY_INT_PXIEN &= ~GM_DRDY_INT_IE; )
...
#define TEN308_DIOA_GPIO				BV(6)//BV(3)
#define TEN308_DIOA_PIN					P0_6//P1_3
...
static void disable_gm_int(void);
static void enable_gm_int(void);
...
/*		RFwakeup();
		...
		if (GM_dev_init() == TRUE)
		{
			...
			osal_start_reload_timer(task_id,HEART_BEAT_EVT,hrtbt_timeout);
		}
		else
...
static void read_gm_data(void)
{
	...
	else
	{
		...
		gm_data_proc(gdetmpr,xval,yval,zval);
	}
	// RF have data send
	if (RFrxtx == TRUE)
...
{
	HAL_PIN_INT_DISABLE();
	GM_DRDY_INT_INT_DISABLE();
}

static void enable_gm_int(void)
{
	GM_DRDY_INT_INT_ENABLE();
	HAL_PIN_INT_ENABLE();
}
...
static void GM_DRDY_INT_Cfg(void)
{
	...
	//Confige CC2541 pin
	GM_DRDY_INT_PINSEL &= ~GM_DRDY_INT_IE;
	GM_DRDY_INT_PINDIR &= ~GM_DRDY_INT_IE;
...
#endif
{
	...
	if ((GM_DRDY_INT_PXIFG & GM_DRDY_INT_IE) && (GM_DRDY_INT_PXIEN & GM_DRDY_INT_IE))
		osal_start_timerEx(BLECore_TaskId, GM_DRDY_INT_INT_EVT,10);
