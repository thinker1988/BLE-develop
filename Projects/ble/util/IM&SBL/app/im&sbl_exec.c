...
static void sbl_Proc(void);
static uint8 sbl_Resp(void);
...
uint8 sbl_Poll(void)
{
	...
	while (HalUARTRead(0, &ch, 1))
	{
		switch (rpcSte)
		{
			...
			{
				sbl_Proc();
				return sbl_Resp();	// Send the SB response setup in the sbBuf passed to sblProc().
			}
			break;
...
static void sbl_Proc(void)
{
	...
	uint16 t16 = BUILD_UINT16(sbBuf[SBL_REQ_ADDR_LSB], sbBuf[SBL_REQ_ADDR_MSB]) + IMG_B_BEG_FLASH_ADDR;
...
/*			len = SBL_RW_BUF_LEN + SBL_READ_HDR_LEN;
			...
			HalFlashRead(t16 / SBL_PAGE_SIZE, (t16%SBL_PAGE_SIZE)<<2, (sbBuf+SBL_RSP_DAT0), SBL_RW_BUF_LEN);
			*/
		break;

		case SBL_ENABLE_CMD:
			HalFlashRead(BIM_IMG_B_PAGE, BIM_CRC_OSET, (uint8 *)crc, sizeof(crc));
			...
			if (crc[1] != crc[0])
			{
				crc[1] = crc[0];
				crc[0] = 0xFFFF;
				HalFlashWrite(IMG_B_BEG_FLASH_ADDR, (uint8 *)crc, 1);
				HalFlashRead(BIM_IMG_B_PAGE, BIM_CRC_OSET, (uint8 *)crc, sizeof(crc));
			}
			// Bootload master must have verified extra checks to be issuing the SBL_ENABLE_CMD.
			if (crc[0] != crc[1])
